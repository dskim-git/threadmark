...
export type ImportedPaper = {
   ...
   *   bibtex    사이트가 내려준 BibTeX를 읽은 값. 추측이 없다.
   *   ris       사이트가 내려준 RIS를 읽은 값. 추측이 없다.
   *   ai        AI가 첫 장을 읽어 뽑은 값. **틀릴 수 있다.**
   *
   * 화면이 이 값으로 경고의 세기를 달리한다. 앞의 셋은 이미 조각으로 나뉜
   * 값을 옮겨 담은 것이고, 마지막 하나만 읽어내는 과정이 끼어 있다.
   * 넷을 같은 얼굴로 보여주면 어느 쪽을 더 눈여겨봐야 하는지 알 수 없다.
   */
  source: "crossref" | "bibtex" | "ris" | "ai";
};
