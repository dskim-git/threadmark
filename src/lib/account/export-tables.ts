...
export const EXPORTED_TABLES = [
  ...
  */
  "ai_usage_events",
  /*
    한도에 더해받은 허용량. (19-E) **이용자에 관한 기록이므로 담는다.**

    `admin_audit_logs`와 갈리는 자리다. 그쪽은 **우리가 무엇을 했는가**이고
    이쪽은 **이 사람이 몇 번을 더 쓸 수 있었는가**다. "왜 내가 100번을
    쓸 수 있었나"에 답하는 값이다.

    `reason`은 운영자가 적은 글이다. 담는 것이 맞다. 이용자는 이미 앱에서
    그 글을 볼 수 있고(자기 허용량을 읽는 정책이 있다), **내려받는 파일에만
    빼면 앱에서 본 것과 파일이 어긋난다.**
  */
  "ai_usage_grants",
] as const;
