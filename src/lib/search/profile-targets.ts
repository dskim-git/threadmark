 ...
 *
 * **이 목록을 두 검색이 함께 본다.** `글자로 찾기`(`search/queries.ts`)와
 * `AI에게 물어보기`(`ai/candidates.ts`)다. 그래서 어느 한쪽 아래가 아니라
 * 검색 밑에 둔다.
 *
 * 무엇이 빠져 있었나
 ...
 *
 * 한쪽만 고쳐져 있었다 (2026-09-26)
 *   만들 때는 `AI에게 물어보기`만 이 목록을 썼다. 그래서 이 파일이 한동안
 *   `lib/ai/` 아래 있었고, `글자로 찾기`는 자료 제목과 기록 글만 보고
 *   있었다.
 *
 *   AI에게 물으면 가수 이름으로 찾아주는데 `글자로 찾기`는 못 찾았다.
 ...
 *
 *   **파일을 옮긴 것이 그 고침의 마무리다.** 목록이 `lib/ai/` 아래 있으면
 *   다음에 표를 만드는 사람이 그곳을 뒤질 이유가 없다. 이름과 자리가
 *   무엇을 위한 것인지 말해야 한다.
 *
