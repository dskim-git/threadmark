 ...
 * **무엇을 접고 무엇을 펼쳐둘지의 기준.**
 *
 *   그 화면에 온 이유면 펼쳐둔다. 곁가지면 접는다.
 *
 * 받은함의 `새 기록`은 접지 않는다. 그 화면에 오는 이유가 그것이다.
 * 자료 화면의 `프로젝트에 잇기`나 `파일 붙이기`는 접는다. 자료 화면에
 * 오는 이유는 읽고 보는 것이고, 잇는 일은 가끔 있다.
 *
 * **자주 하는 일은 화면을 많이 써야 한다.** 한 번 더 누르게 만들면 그만큼
 * 덜 하게 된다. 반대로 가끔 하는 일이 화면을 차지하면 자주 보는 것이 아래로
 * 밀린다. 처음에는 그 반대로 되어 있었다.
 */
