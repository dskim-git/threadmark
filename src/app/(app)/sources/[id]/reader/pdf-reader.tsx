    ...
    */
    <div className="flex h-full min-h-0 flex-col gap-3">
      <div className="flex shrink-0 flex-wrap items-center gap-2 rounded-xl border border-black/[.08] px-3 py-2 dark:border-white/[.145]">
        ...
        {/*
          안내를 도구 줄 오른쪽 끝에 붙인다. 제 줄을 쓰지 않으므로 읽는 자리를
          먹지 않는다. 자리가 좁으면 감춘다. 몰라도 단추로 다 할 수 있는 일이다.
        */}
        <span className="ml-auto hidden text-xs text-zinc-500 xl:inline">
          좌우 방향키로도 넘길 수 있습니다
        </span>
      </div>
      ...
      */}
      ...
      {/*
        안내문을 여기서 뺐다. 세 줄짜리 문단이 PDF 아래에 늘 붙어 있었는데,
        이 화면은 창 높이를 재서 남는 만큼을 PDF에 주므로 그만큼 읽는 자리가
        줄었다.

        드래그해서 인용을 남긴다는 말은 오른쪽 `메모` 탭이 이미 하고 있다.
        방향키 이야기는 도구 줄로 옮겼다. 파일이 Drive에 있다는 것은 자료
        화면과 설정에 적혀 있다.
      */}
    </div>
  );
