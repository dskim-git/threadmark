...
- **뼈대 서식(템플릿).** 수업은 도입-전개-정리, 논문은 서론-이론-방법-결과가
  반복된다. 다만 몇 번 만들어 보고 무엇이 반복되는지 안 뒤에 만든다.
  지금 만들면 추측한 서식이 된다.
- **메뉴를 두 축으로 나누기.** 지금 메뉴는 평평해서 모으는 일과 만드는 일이
  같은 무게로 보인다. 다만 메뉴를 바꾸는 것은 쓰던 사람이 길을 잃는 변화라,
  뼈대를 써본 뒤에 이야기한다.
- **읽다가 바로 자리에 보내기.** 논문을 읽으며 문장을 고르는 순간이 "이건
  2장에 쓰겠다"가 떠오르는 순간이다. 그때 기록 창에서 자리를 고를 수 있으면
  나중에 배치하는 일 자체가 줄어든다.
